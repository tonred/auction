...
class EnglishForwardAuctionTest(EnglishAuctionTest):
    ...
    def test_increasing_bids(self):
        ...
        self._send_tip3(user_1, value=3)
        self._send_tip3(user_2, value=5)
        self._send_tip3(user_3, value=12)
    ...
    def test_low_bids_start(self):
        self._setup_phase_time(Phase.OPEN, update=True)
        user = self._create_user()
        self._send_tip3(user, value=1)  # too low bid
    ...
    def test_low_bids_step(self):
        self._setup_phase_time(Phase.OPEN, update=True)
        user = self._create_user()
        self._send_tip3(user, value=4)  # good bid
        self._send_tip3(user, value=5)  # too low step
        self._check_bids_count(1)
    ...
    def test_bid_return(self):
        ...
        self._send_tip3(user_1, value=3)  # first bid
        self.assertEqual(100 - 3, user_1.tip3_balance, 'Bid is not made')

        self._send_tip3(user_2, value=5)  # second bid
