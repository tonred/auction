...
class BlindAuctionTest(BaseAuctionTest):
    ...
    def test_bid_in_open_phase(self):
        ...
        self._make_bid(user, value=10, bid_value=5, salt=salt)
        self._check_bids_count(1)
        self.assertEqual((100 - 10) * ts4.GRAM, user.ton_balance, 'Bid is not made')
    ...
    def test_remove_bid(self):
        ...
        self._remove_bid(user, value=1, bid_value=5, salt=salt)
        self._check_bids_count(0)  # bid really removed
        self.assertEqual((100 - 1) * ts4.GRAM, user.ton_balance, 'Bid is not removed')

    def test_low_bid_value(self):
        self._setup_phase_time(Phase.OPEN, update=True)
        user = self._create_user()
        salt = self._generate_salt()
        self._make_bid(user, value=9, bid_value=5, salt=salt, expect_ec=Errors.VALUE_LESS_THAN_DEPOSIT)
        self._check_bids_count(0)
        self.assertEqual(100 * ts4.GRAM, user.ton_balance, 'Bid is not made')

    def _wrong_phase_bid_test(self):
        user = self._create_user()
        salt = self._generate_salt()
        self._make_bid(user, value=10, bid_value=5, salt=salt, expect_ec=Errors.WRONG_PHASE)
        self._check_bids_count(0)
        self._check_confirmed_bids_count(0)
        self.assertEqual(100 * ts4.GRAM, user.ton_balance, 'Bid is not returned')
    ...
    # not operated with tip3 tokens
    def _make_bid(self, user: User, value: float, bid_value: int, salt: int, expect_ec: int = 0):
        ...
        user.ton_wallet.send_call_set(
            destination, value, call_set=call_set, abi=self.contract.abi_, expect_ec=expect_ec
        )

    # not operated with tip3 tokens
    def _remove_bid(self, user: User, value: float, bid_value: int, salt: int, expect_ec: int = 0):
        destination = self.contract.address
        hash_ = self._calc_hash(bid_value, salt)
        value = int(value * ts4.GRAM)
    ...
    def _confirm_bid(self, user: User, bid_value: int, salt: int):
        payload = self.contract.call_getter('buildConfirmPayload', {
            'value': bid_value,
            'salt': salt,
        })
        self._send_tip3(user, value=bid_value, payload=payload.raw_)
